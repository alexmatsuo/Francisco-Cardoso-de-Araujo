...
export default function WorkDetailPage() {
  ...
  const fetchWork = async () => {
    try {
      const response = await fetch(`/api/works/large-ensembles/${params.slug}`);
  ...
  if (!work) {
    return (
      <main className="p-8 max-w-4xl mx-auto">
        ...
        <Link href="/works/large-ensembles" className="text-[#D3CEAD] hover:underline mt-4 inline-block">
          ← Back to Large Ensembles Works
        </Link>
      </main>
    );
  }
  ...
  return (
    <main className="p-8 max-w-6xl mx-auto">
      <Link href="/works/large-ensembles" className="text-[#D3CEAD] hover:underline mb-6 inline-block">
        ← Back to Large Ensembles Works
      </Link>

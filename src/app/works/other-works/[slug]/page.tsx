...
interface Work {
  ...
  title: string;
  year: number;
  instruments: string; // Type field (Poetry, Visual Art, etc.)
  duration: string | null;
  information: string | null;
  programNotes: string | null; // Artist statement / Description
...
export default function OtherWorkDetail({ params }: { params: { slug: string } }) {
  ...
  const fetchWork = async () => {
    try {
      ...
      const foundWork = data.works?.find((w: Work) => w.slug === params.slug);
      
      const processedWork = foundWork ? {
        ...foundWork,
        videoUrls: Array.isArray(foundWork.videoUrls) ? foundWork.videoUrls : []
      } : null;
  ...
  if (!work) {
    return (
      <main className="p-8 max-w-4xl mx-auto">
        ...
        <Link href="/works/other-works" className="text-[#D3CEAD] hover:underline mt-4 inline-block">
          ← Back to Other Works
        </Link>
      </main>
    );
  ...
  return (
    <main className="p-8 max-w-6xl mx-auto">
      <Link href="/works/other-works" className="text-[#D3CEAD] hover:underline mb-6 inline-block">
        ← Back to Other Works
      </Link>
